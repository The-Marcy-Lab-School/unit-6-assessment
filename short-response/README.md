...
Answer: Async functions are functions that returns an Async Function object. Their relationship to Promises is, when an async function operate , aside from the rest of the code in the event loop, the value that is return when this asycn function is finished is a Promise Object. Async/Await introduce code readabilty.

4. Consider one of your favorite apps. Describe one feature of that app that is likely powered by AJAX.

Answer: One of my favorite app is Twitch. One feature of that app that is likely powered by AJAX is the viewing different content (pages) while a video is being played or minimized in the corner. I am able to able on different pages and read bio's or even watch a different video while a video is being loaded in the background .